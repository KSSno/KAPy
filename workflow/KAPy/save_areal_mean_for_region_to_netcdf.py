...
def save_areal_mean_for_region_to_netcdf(config: dict, indicator_id: str, scenario: str, period: str, region: str, inputs: list[str], output: str):
    ...
    for input_file in inputs:
        ...
        print(f"clipped_ds = {clipped_ds}")
        region_datasets[input_file] = clipped_ds
    print(f"region datasets: {region_datasets}\n")
    ...
    if not datasets: # doesn't work for daily datasets, have to extract time period
        ...
        year = arrow.get(period_config["start"])
        end = arrow.get(period_config["end"])
        
        while year <= end:
            datasets = [region_datasets[filename] for filename in region_datasets if year.format("YYYY") in filename] 
